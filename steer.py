...
def _get_token_count(tts: Qwen3TTSModel, instruct: str) -> int:
...
def extract_hidden(
    ...
    """
    hidden_outputs: Dict[int, torch.Tensor] = {l: None for l in target_layers}
    handles: list = []
    ...
    if hook_mode == "pa":
        ...
        def _hook_factory(l: int):
            def hook(module, input, output):
                if input[0].shape[1] > 1:  # prefill only
                    hidden_outputs[l] = input[0].detach().clone()

            return hook

        for l in target_layers:
            h = tts.model.talker.model.layers[
                l
            ].post_attention_layernorm.register_forward_hook(_hook_factory(l))
            handles.append(h)
        ...
        def _hook_factory(l: int):
            def hook(module, input, output):
                if input[0].shape[1] > 1:
                    hidden_outputs[l] = output[0].detach().clone()
        ...
        raise NotImplementedError(f"Not support hook_mode={hook_mode}")
    ...
    try:
        ...
        return {l: hidden_outputs[l][0, -1, :] for l in target_layers}
...
def calibrate(
    ...
    *lang* has no effect when *calibration_texts* is given explicitly.
    ...
    for ct in calibration_texts:
        h_strong = extract_hidden(
            tts, instruct, target_layers, hook_mode=hook_mode, calibration_text=ct
        )
        h_neutral = extract_hidden(
            tts, "", target_layers, hook_mode=hook_mode, calibration_text=ct
        )
        for l in target_layers:
...
def generate(
    ...
    scale: float,
    hook_mode: str = "pa",
    steer_generate_scale: float = 0.0,
):
    ...
    When *steer_generate_scale* > 0, steering is also applied at every
    auto-regressive generate step (to the single codec token position),
    continuously nudging the prediction toward the instruction direction.
    ...
    if hook_mode == "pa":
        ...
        def _hook_factory(layer: int, scale: float, n_inst: int, scale_gen: float):
            sv = steerings[layer] / (steerings[layer].norm() + 1e-8)

            def pre_hook(module, input):
                seq_len = input[0].shape[1]
                if seq_len > 1:
                    input[0][:, :n_inst, :] += scale * sv.to(
                        device=input[0].device, dtype=input[0].dtype
                    )
                elif scale_gen != 0 and seq_len == 1:
                    input[0][:, 0, :] += scale_gen * sv.to(
                        device=input[0].device, dtype=input[0].dtype
                    )
        ...
        for l in steering_layers:
            h = tts.model.talker.model.layers[
                ...
                _hook_factory(l, scale, n_inst, steer_generate_scale)
            )
            handles.append(h)
    else:
        ...
        def _hook_factory(layer: int, scale: float, n_inst: int, scale_gen: float):
            ...
            def hook(module, input, output):
                seq_len = input[0].shape[1]
                if seq_len > 1:
                    output[0][:, :n_inst, :] += scale * sv.to(
                        device=output[0].device, dtype=output[0].dtype
                    )
                elif scale_gen != 0 and seq_len == 1:
                    output[0][:, 0, :] += scale_gen * sv.to(
                        device=output[0].device, dtype=output[0].dtype
                    )
